	...
	var e = $("body"),
			...
			switch (e) {
				...
				case 7:
					var s = 2.273,
						...
						d;
					if (l < 1000) {
						d = (-0.45 * ((l / 100) * (l / 100)) + 9 * (l / 100) + 5).toFixed(1);
					} else if (l >= 1000 && l < 2000) {
						d = (0.45 * ((l / 100) * (l / 100)) - 9 * (l / 100) + 95).toFixed(1);
					} else {
						d = 99.9;
					};
					console.log(f, o)
					switch (true) {
						case f < o:
							...
							$("#finish .score").html(f);
							$("#finish .play-percent").html(d + "%");
							$("#medal-name").css("color", "#ffffff");
							$("#medal-name").html("玩的不错！");
							//$(".next-medal").show();
						...
						case f < u:
							...
							$("#finish .score").html(f);
							$("#finish .play-percent").html(d + "%");
							$("#medal-name").css("color", "#d79960");
							$("#medal-name").html("获得铜牌！");
						...
						case f < a:
							...
							$("#finish .score").html(f);
							$("#finish .play-percent").html(d + "%");
							$("#medal-name").css("color", "#cecece");
							$("#medal-name").html("获得银牌！");
							//$("#finish .next-medal").show();
						...
						case f >= a:
							...
							$("#finish .score").html(f);
							$("#medal-name").css("color", "#ffd675");
							$("#medal-name").html("获得金牌！");
							//$("#finish .play-percent").html(d + "%");
		...
		h = function(r) {
			var a = [],
				...
				d = [
					[-80, 0, 80, 0], //begin
					[-73, -70, -42, 68, -12, 68, 0, 18, 12, 68, 42, 68, 73, -70, 43, -70, 28, 22, 0, -60, -28, 22, -43, -70], //wow-logo
					[-14, -67, 25, -64, 52, -48, 90, 0, 142, 0, 131, 40, 142, 81, 95, 75, 88, 115, 33, 83, -40, 85, -94, 25, -140, 10, -117, -17, -136, -49, -138, -99, -57, -64], //bb
					[-100, -141, -130, -146, -127, -110, -92, -60, -95, -28, -55, 58, -20, 150, 61, 126, 133, 115, 48, 58, 12, 80, -27, 41, -15, -9, -71, -71, -88, -100], //sword
					[0, -150, 42, -88, 85, -97, 88, -42, 150, 0, 88, 42, 85, 97, 42, 88, 0, 150, -42, 88, -85, 97, -88, 42, -150, 0, -88, -42, -85, -97, -42, -88], //hs-logo
					[-65, 75, -38, 119, -72, 126, -130, 50, -110, 25, -50, -90, -30, -120, 0, -138, 60, -115, 82, -73, 115, 15, 135, 65, 113, 95, 90, 128, 88, 75, 60, -5, 10, -60, -37, -12], //siv
					[0, 0, 25, 25]
				];
			var v = function() {
			...
			var w = function() {
				...
				var l = function(t) {
					...
					p.push(n($(t).position().left + 12, $(t).position().top + 12, $(t).position().left + 12, $(t).position().top + 12, $(t).position().left + 12, $(t).position().top + 12, "#111111"));
					var l = p[p.length - 1];
				...
				if (t.hasClass("touch")) {
					...
					$(a).each(function(t, r) {
						$(r).on("mousedown", function(t) {
							...
							p.push(n($(r).position().left + 12, $(r).position().top + 12, t.pageX, t.pageY, t.pageX, t.pageY, "#111111"));
							var c = p[p.length - 1];
		...
		p = function() {
			...
			var t = ["img/img_dot.png", "img/img_dot-full.png", "img/img_end.png", "img/img_end-br.png", "img/img_end-si.png", "img/img_end-go.png", "img/img_retry_c.png", "img/img_play-again.png"];
...
function yixinShare(_shareData) {
	var appkey="",
		type="webpage",
		text="dotGame!!!",
		t=encodeURI(text),
		userdesc="我"+$("#medal-name").html()+"最终得分："+$(".score").html(),
		pic="",
		url="http://wow.tgbus.com/special/testyu/c/";

	var u = "http://open.yixin.im/share?url="+ url + "&pic=" + pic + "&userdesc=" + userdesc + "&text=" + t;
	window.open(u);
};